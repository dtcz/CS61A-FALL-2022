...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not current.is_hive:
            if count >= self.lower_bound and count <= self.upper_bound and len(current.bees):
        ...
        if place:
            if self.health <= 0:
                for bee in list(place.bees):
                    bee.reduce_health(current_health + self.damage)
            else:
                for bee in list(place.bees):
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.turns_to_chew:
            ...
            if self.place and len(self.place.bees):
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        while current and not current.is_hive:
            if current.ant:
                current.ant.double()
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        target_slowed = hasattr(target.action, "is_slowed")
        if target_slowed:
            target.action.turns_until_speedup = 5
            return
        ...
        def slow_action(gamestate):
            ...
            if hasattr(slow_action, "turns_until_speedup"):
                if slow_action.turns_until_speedup > 0:
                    slow_action.turns_until_speedup -= 1
                elif slow_action.turns_until_speedup == 0:
                    target.action = original_action

        original_action = target.action
        slow_action.turns_until_speedup = 5
...
class LaserAnt(ThrowerAnt):
    ...
    name = 'Laser'
    food_cost = 10
    # OVERRIDE CLASS ATT RIBUTES HERE
    ...
    def insects_in_front(self):
        ...
        while current and not current.is_hive:
            if current.ant and current.ant is not self:
                front[current.ant] = distance
            for bee in list(current.bees):
                front[bee] = distance
            current = current.entrance
            distance += 1
        return front
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        return max(0, 2 - 0.25 * distance - 0.0625 * self.insects_shot)
        # END Problem Optional 2

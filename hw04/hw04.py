...
def balanced(m):
    ...
    return balanced(end(left_arm)) and balanced(end(right_arm)) and total_weight(end(left_arm)) * length(left_arm) == total_weight(end(right_arm)) * length(right_arm)
...
def has_path(t, word):
    ...
    if len(word) > 1:
        ...
        for branch in branches(t):
            res = res or has_path(branch, word[1:])
        return res
    return True

...
def partition_gen(n):
    ...
    def yield_helper(j, k):
        if k == 0:
            ...
            for p in yield_helper(j - k, min(k, j - k)):
                yield [k] + p
...
def long_paths(t, n):
    ...
    def dfs(tree, curr, len):
        nonlocal res
        if tree.is_leaf():
            if len >= n:
                res.append(curr + [tree.label])
            return
        for i in tree.branches:
            dfs(i, curr + [tree.label], len + 1)
    dfs(t, [], 0)
    return res

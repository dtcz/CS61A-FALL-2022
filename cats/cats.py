...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    return lambda p: any([x in split(lower(remove_punctuation(p))) for x in topic])
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    if typed_words_len == 0 and source_words_len == 0:
        return 100.0
    if typed_words_len == 0 or source_words_len == 0:
        return 0.0
    count = 0
    for i in range(min(typed_words_len, source_words_len)):
        if typed_words[i] == source_words[i]:
            count = count + 1
        i = i + 1
    return count / typed_words_len * 100.0
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    ret, max = typed_word, -1
    for word in word_list:
        if typed_word == word:
            return typed_word
        diff = diff_function(typed_word, word, limit)
        if (max == -1 and diff <= limit) or diff < max:
            ret = word
            max = diff
    return ret
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    m = min(len(typed), len(source), limit + 1)
    ...
    def helper(a, b, i):
        ...
        if i >= m:
            return 0
        return int(a[i] != b[i]) + helper(a, b, i + 1)
    return ret + helper(typed, source, 0)
    # END PROBLEM 6
...
def final_diff(typed, source, limit):
    ...
    if len(typed) == 0 or len(source) == 0:  # Fill in the condition
        # BEGIN
    ...
    elif len(typed) > 1 and len(source) > 1 and typed[0] != source[0] and typed[0] == source[1] and typed[1] == source[0]:
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            time_ins = time(match, j, i)  # _ins  实例
